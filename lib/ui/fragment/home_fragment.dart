...
class _HomeFragmentState extends State<HomeFragment> {
  ...
  Future<void> initVPNPlatformState({required Server server}) async {
    engine.connect(
        utf8.fuse(base64).decode(server.address!), server.name ?? "OvO Server",
        username: "", password: "");
    if (!mounted) return;
  }
  ...
  void connect({required VpnController controller}) {
    if (controller.haveVpn == true) {
      ...
      if (stage.toString() == VPNStage.disconnected.toString() ||
          stage.toString() == "null") {
        initVPNPlatformState(server: controller.server!);
                                          ...
                                          children: [
                                            ...
                                            Padding(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Image.asset(
                                                      "assets/flag/${controller.server!.category?.code ?? "US"}.png",
                                                      height: 35),
                                                  const SizedBox(
                                                    height: 10,
                                                  ),
